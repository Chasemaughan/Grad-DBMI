...
<br> \#### Another way to create a new variable is with the mutate() function from the tidyverse package in this case we are going to convert the Age in years to Age in months <br>
...
<br> \#### ggplot2 is a great package included in the tidyverse group, let's use it to create a histogram of the Age distribution <br>
...
#### 2. How many patients have diabetes?<br>  #268

```{r if_else}
diabetes %>%  group_by(Outcome) %>% tally()
```

#### 3. How many patients have diabetes that are older than 45?<br> #58

```{r table_complex_tidyverse}
diabetes %>% filter(Age >= 46 ) %>% group_by(Outcome) %>% tally()

```

#### 4. What is the mean and variance of glucose levels for individuals without diabetes<br>   #109 and 26.142

```{r Summary_Age}
summary_glu <- diabetes %>% filter(Outcome == 0 ) %>% 
  summarize(mean = mean(Glucose), variance = sd(Glucose))
summary_glu
```
...
```{r Plot_hist}
ggplot(data = diabetes %>% filter(Outcome == 0), mapping = aes(x = BMI)) +
  geom_histogram(color = "white", bins = 25)+
   ggtitle("BMI Distribution for Individuals Without Diabetes") + theme_classic()
ggplot(data = diabetes %>% filter(Outcome == 1), mapping = aes(x = BMI)) +
  geom_histogram(color = "white", bins = 25)+
   ggtitle("BMI Distribution for Individuals With Diabetes") +  theme_classic()
